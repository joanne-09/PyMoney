...
class Records:
    ...
    def delete(self):
        ...
        for i in range(len(self)-1, -1, -1):
            if self._records[i][0]==tmp[0] and self._records[i][1]==tmp[1] and self._records[i][2]==tmp[2]:
                self._records.pop(i)
                return
            ...
            if type(cate) == list:
                for v in cate:
                    ...
                    if p == True:
                        ...
                        index = cate.index(v)
                        return Records._flatten(cate[index:index + 2])
                    if p != False:
                        # p is a list returned from flatten
                        return p
            return cate == item

        return find(item, iniCate)
